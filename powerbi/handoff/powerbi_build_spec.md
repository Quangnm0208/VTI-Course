...
**Nguồn:** Stack Overflow Developer Survey 2025 (49.191 dev / 177 quốc gia / median 75.320) **+** ItViec (1.200 JD, scrape 2026-05-28)
...
```DAX
Total Respondents = CALCULATE(MAX(dataset_overview[value]), dataset_overview[metric]="Total respondents")   -- 49.191
Total Countries   = CALCULATE(MAX(dataset_overview[value]), dataset_overview[metric]="Countries covered")   -- 135
Median Salary     = CALCULATE(MAX(dataset_overview[value]), dataset_overview[metric]="Median salary USD")   -- 75.320
...
| **So sánh lương VN vs toàn cầu** | **Bar phân khoảng** (min→max) + đường tham chiếu | vn_salary_benchmark (annual_usd_min/max); reference line tại 75.320 |
...
6. Đối chiếu với `reference/dashboard_reference.html` (49.191 · 135 · JavaScript · PostgreSQL · DevOps $71.036 · VN Python/AI 19,3% · VN senior ~52% mốc toàn cầu).
